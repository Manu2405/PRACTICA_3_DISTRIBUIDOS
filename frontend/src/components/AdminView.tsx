...
type AdminViewProps = {
  ...
  eventLogs: EventLog[];
  aircrafts: Aircraft[];
  dashboard?: ApiAdminDashboardResponse | null;
  brandShort: string;
  brandName: string;
};
...
function ClusterNode({ node, index, total, t, lang }: { node: NodeStatus; index: number; total: number; t: any; lang: Language }) {
  ...
  const uptime = Math.max(0, 100 - node.load).toFixed(2);
      ...
      <div className="relative w-20 h-20 rounded-2xl bg-slate-800 border border-slate-700 shadow-xl flex flex-col items-center justify-center gap-1 group-hover:scale-110 group-hover:border-cyan-500/50 transition-all z-10 hover:z-20">
         ...
         <div className="absolute -bottom-24 left-1/2 -translate-x-1/2 w-32 bg-slate-950/90 border border-white/10 p-2 rounded-xl scale-0 group-hover:scale-100 transition-transform pointer-events-none backdrop-blur-md z-30 shadow-2xl">
           <div className="space-y-1">
               <div className="flex justify-between text-[8px] text-slate-400"><span>{t.latency}:</span> <span className="text-cyan-400 font-mono">{node.synced}</span></div>
...
function ProtocolLog({ eventLogs, t }: { eventLogs: EventLog[]; t: any }) {
  const logs = eventLogs.slice(0, 8);
  ...
  return (
    <div className="h-full bg-slate-950/80 border border-slate-800 rounded-3xl p-4 font-mono text-[10px] overflow-hidden flex flex-col">
       ...
       <div className="space-y-2 flex-grow overflow-y-auto custom-scrollbar pr-2">
          {logs.map((log) => (
            <div key={`${log.time}-${log.event}`} className="flex gap-2 animate-fade-in-right text-slate-400">
...
export default function AdminView({
  ...
  const lastSyncLabel = dashboard?.sync.mongo.latestEventUtc
    ? formatSyncLabel(dashboard.sync.mongo.latestEventUtc)
    : dashboard?.generatedAt
      ? formatSyncLabel(dashboard.generatedAt)
      : lastUpdate;
  ...
  const backendEvents = dashboard?.sync.mongo.recentEvents ?? eventLogs;
  ...
  const globalStats = useMemo(() => {
    if (dashboard) {
      return {
        totalFlights: dashboard.flights.total,
        soldSeats: dashboard.seats.sold,
        reservedSeats: dashboard.seats.reserved,
        totalIncomeFirst: dashboard.sales.firstClassRevenueUsd,
        totalIncomeEconomy: dashboard.sales.economyRevenueUsd,
        totalCapacity: dashboard.seats.total,
        availability: dashboard.seats.free,
      };
    }
  ...
  }, [dashboard, filteredRoutes, aircrafts]);
  ...
  const syncStats = useMemo(() => {
    if (dashboard) {
      return {
        syncPercent: dashboard.sync.alignmentPct,
        syncedNodes: dashboard.sync.alignedTables,
        totalNodes: dashboard.sync.totalTables,
        avgLoad: dashboard.sync.alignmentPct,
        latestSync: dashboard.generatedAt,
        activeConflicts: conflicts.length,
        recentEvents: dashboard.sync.mongo.totalEvents,
      };
    }

    const totalNodes = nodeStatuses.length || 1;
    const syncedNodes = nodeStatuses.filter((node) => /sincronizado/i.test(node.status)).length;
    const avgLoad = Math.round(
      nodeStatuses.reduce((sum, node) => sum + node.load, 0) / totalNodes,
    );
    const latestSync = nodeStatuses[0]?.synced ?? '--';

    return {
      syncPercent: Math.round((syncedNodes / totalNodes) * 1000) / 10,
      syncedNodes,
      totalNodes,
      avgLoad,
      latestSync,
      activeConflicts: conflicts.length,
      recentEvents: eventLogs.length,
    };
  }, [dashboard, nodeStatuses, conflicts.length, eventLogs.length]);
    ...
    <section className="space-y-6 animate-fade-in-up pb-10">
      ...
      <div className="flex flex-wrap gap-2 rounded-2xl border border-white/10 bg-slate-900/60 p-2">
        ...
        <div className="ml-auto flex items-center gap-2 rounded-xl bg-slate-800/80 px-3 py-2 text-[10px] text-slate-400 border border-white/5">
          ...
          <span className="font-bold tabular-nums">{t.last_sync}: {lastSyncLabel}</span>
        </div>
      </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
           ...
           <div className="space-y-6 flex flex-col">
              ...
              <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-xl flex flex-col items-center gap-4">
                 ...
                 <div className="relative w-36 h-36">
                   ...
                   <div className="absolute inset-0 flex flex-col items-center justify-center">
                       <span className="text-3xl font-black text-white tabular-nums">{syncStats.syncPercent}%</span>
                       <span className="text-[8px] font-bold text-emerald-400 uppercase">{t.optimal}</span>
                    </div>
                 </div>
                 <div className="grid w-full grid-cols-2 gap-3 text-[10px] font-bold uppercase tracking-widest text-slate-500">
                    <div className="rounded-2xl border border-white/5 bg-slate-950/60 p-3">
                       <p>{t.nodes}</p>
                       <p className="mt-1 text-sm text-white tabular-nums">{syncStats.syncedNodes}/{syncStats.totalNodes}</p>
                    </div>
                    <div className="rounded-2xl border border-white/5 bg-slate-950/60 p-3">
                       <p>{dashboard ? 'Consistencia' : t.latency}</p>
                       <p className="mt-1 text-sm text-cyan-400 tabular-nums">{syncStats.avgLoad}%</p>
                    </div>
                    <div className="rounded-2xl border border-white/5 bg-slate-950/60 p-3 col-span-2">
                       <p>{t.last_sync}</p>
                       <p className="mt-1 text-sm text-emerald-400 tabular-nums">{syncStats.latestSync}</p>
                    </div>
                    <div className="rounded-2xl border border-white/5 bg-slate-950/60 p-3">
                       <p>{t.active_conflicts}</p>
                       <p className="mt-1 text-sm text-rose-400 tabular-nums">{syncStats.activeConflicts}</p>
                    </div>
                    <div className="rounded-2xl border border-white/5 bg-slate-950/60 p-3">
                       <p>{t.system_events}</p>
                       <p className="mt-1 text-sm text-cyan-300 tabular-nums">{syncStats.recentEvents}</p>
                    </div>
                 </div>
              </div>

              {dashboard && (
                <div className="rounded-3xl border border-cyan-500/20 bg-slate-900 p-6 shadow-xl space-y-4">
                  <div>
                    <h3 className="text-xs font-black text-cyan-400 uppercase tracking-widest">Sincronizacion real</h3>
                    <p className="mt-1 text-sm text-slate-400">SQL principal, secundaria y Mongo con datos reales del backend.</p>
                  </div>
                  <div className="grid grid-cols-2 gap-3 text-[10px] font-bold uppercase tracking-widest text-slate-500">
                    <div className="rounded-2xl border border-white/5 bg-slate-950/60 p-3">
                      <p>Tablas alineadas</p>
                      <p className="mt-1 text-sm text-white tabular-nums">{dashboard.sync.alignedTables}/{dashboard.sync.totalTables}</p>
                    </div>
                    <div className="rounded-2xl border border-white/5 bg-slate-950/60 p-3">
                      <p>Coincidencia</p>
                      <p className="mt-1 text-sm text-emerald-400 tabular-nums">{dashboard.sync.alignmentPct}%</p>
                    </div>
                    <div className="rounded-2xl border border-white/5 bg-slate-950/60 p-3">
                      <p>Mongo events</p>
                      <p className="mt-1 text-sm text-cyan-300 tabular-nums">{dashboard.sync.mongo.totalEvents}</p>
                    </div>
                    <div className="rounded-2xl border border-white/5 bg-slate-950/60 p-3">
                      <p>Tasa ok</p>
                      <p className="mt-1 text-sm text-amber-300 tabular-nums">{dashboard.sync.mongo.successRatePct}%</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    {dashboard.sync.tableSnapshots.map((snapshot) => (
                      <div key={snapshot.table} className="rounded-2xl border border-white/5 bg-slate-950/60 px-3 py-2 text-[11px] text-slate-300 flex items-center justify-between gap-3">
                        <span className="font-bold uppercase tracking-widest text-slate-400">{snapshot.table}</span>
                        <span className="font-mono tabular-nums">
                          P:{snapshot.primaryCount} S:{snapshot.secondaryCount} D:{snapshot.deltaCount}
                        </span>
                        <span className={snapshot.aligned ? 'text-emerald-400' : 'text-rose-400'}>
                          {snapshot.aligned ? 'OK' : 'DIF'}
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              <div className="flex-grow">
                 <ProtocolLog eventLogs={backendEvents} t={t} />
              </div>
           </div>
        </div>
      )}
             ...
             <div className="grid gap-6 lg:grid-cols-[0.55fr_0.45fr]">
               <div className="rounded-3xl border border-white/10 bg-slate-900/70 p-6 shadow-xl">
                 <div className="flex items-center justify-between gap-4 mb-5">
                   <h3 className="text-lg font-black text-white uppercase tracking-tighter">Ventas por clase</h3>
                   <span className="text-[10px] text-slate-500 uppercase tracking-widest">{dashboard.sales.totalSales} ventas</span>
                 </div>
                 <div className="space-y-3">
                   {dashboard.sales.byClass.map((entry) => (
                     <div key={entry.classCode} className="rounded-2xl border border-white/5 bg-slate-950/60 p-4">
                       <div className="flex items-center justify-between gap-3">
                         <div>
                           <p className="text-sm font-bold text-white">{entry.className}</p>
                           <p className="text-[10px] text-slate-500 uppercase tracking-widest">{entry.classCode}</p>
                         </div>
                         <div className="text-right">
                           <p className="text-sm font-black text-emerald-400 tabular-nums">${entry.revenueUsd.toLocaleString()}</p>
                           <p className="text-[10px] text-slate-500">{entry.seatsSold} asientos</p>
                         </div>
                       </div>
                     </div>
                   ))}
               ...
               <div className="rounded-3xl border border-white/10 bg-slate-900/70 p-6 shadow-xl">
                 <div className="flex items-center justify-between gap-4 mb-5">
                   <h3 className="text-lg font-black text-white uppercase tracking-tighter">Rutas con demanda</h3>
                   <span className="text-[10px] text-slate-500 uppercase tracking-widest">{dashboard.flights.total} vuelos</span>
                 </div>
                 <div className="space-y-3">
                   {dashboard.flights.topRoutes.map((route) => (
                     <div key={route.flightCode} className="rounded-2xl border border-white/5 bg-slate-950/60 p-4">
                       <div className="flex items-center justify-between gap-3">
                         <div>
                           <p className="text-sm font-bold text-white">{route.flightCode}</p>
                           <p className="text-[10px] text-slate-500 uppercase tracking-widest">{route.route} · {route.status}</p>
                         </div>
                         <div className="text-right">
                           <p className="text-sm font-black text-cyan-300 tabular-nums">{route.occupancyPct}%</p>
                           <p className="text-[10px] text-slate-500">{route.soldSeats}/{route.totalSeats} ocupados</p>
                         </div>
                       </div>
                       <div className="mt-3 h-2 rounded-full bg-slate-800 overflow-hidden">
                         <div className="h-full bg-cyan-500" style={{ width: `${route.occupancyPct}%` }} />
                       </div>
                       <p className="mt-2 text-[10px] text-slate-500 uppercase tracking-widest">Ingresos ${route.revenueUsd.toLocaleString()} · Reservados {route.reservedSeats}</p>
                     </div>
                   ))}
                 </div>
               </div>
      ...
      {tab === 'fleet' && (
        <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-6 shadow-xl backdrop-blur-sm">
          <div className="flex items-center justify-between mb-6">
             ...
             <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest bg-white/5 px-3 py-1 rounded-full border border-white/5">{dashboard?.fleet.totalAircraft ?? aircrafts.length} {t.units}</span>
          </div>
          {dashboard && (
            <div className="grid gap-4 md:grid-cols-4 mb-6">
              <div className="rounded-2xl border border-white/5 bg-slate-950/50 p-4">
                <p className="text-[10px] text-slate-500 uppercase tracking-widest">Flota real</p>
                <p className="mt-2 text-2xl font-black text-white tabular-nums">{dashboard.fleet.totalAircraft}</p>
              </div>
              <div className="rounded-2xl border border-white/5 bg-slate-950/50 p-4">
                <p className="text-[10px] text-slate-500 uppercase tracking-widest">Capacidad total</p>
                <p className="mt-2 text-2xl font-black text-cyan-400 tabular-nums">{dashboard.fleet.totalCapacity}</p>
              </div>
              <div className="rounded-2xl border border-white/5 bg-slate-950/50 p-4">
                <p className="text-[10px] text-slate-500 uppercase tracking-widest">Pasajeros</p>
                <p className="mt-2 text-2xl font-black text-emerald-400 tabular-nums">{dashboard.passengers.total}</p>
              </div>
              <div className="rounded-2xl border border-white/5 bg-slate-950/50 p-4">
                <p className="text-[10px] text-slate-500 uppercase tracking-widest">Con email</p>
                <p className="mt-2 text-2xl font-black text-amber-300 tabular-nums">{dashboard.passengers.withEmail}</p>
              </div>
            </div>
          )}
          {dashboard && (
            <div className="mb-6 rounded-3xl border border-white/10 bg-slate-950/60 p-5">
              <div className="flex items-center justify-between gap-4 mb-4">
                <h4 className="text-sm font-black text-white uppercase tracking-widest">Modelos reales</h4>
                <span className="text-[10px] text-slate-500 uppercase tracking-widest">Backend SQL</span>
              </div>
              <div className="grid gap-3 md:grid-cols-2">
                {dashboard.fleet.models.map((model) => (
                  <div key={model.model} className="rounded-2xl border border-white/5 bg-slate-900/80 p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-bold text-white">{model.model}</p>
                        <p className="text-[10px] text-slate-500 uppercase tracking-widest">Eco {model.economy} / Bus {model.business} / Pri {model.first}</p>
                      </div>
                      <p className="text-lg font-black text-cyan-300 tabular-nums">{model.totalCapacity}</p>
                    </div>
                  </div>
                ))}
              </div>
