...
export var MAP_WORLD_IMAGE = '/mapamundi.jpg';
export var cities = [
...
export function getPlaneColumns(model) {
    if (model.includes('A380'))
        return 10;
    if (model.includes('E195') || model.includes('A318'))
        return 4;
    return 6;
}
/**
 ...
 */
export function generateSeatMatrixForPlane(totalSeats, columns) {
    if (columns === void 0) { columns = 6; }
    var rows = Math.ceil(totalSeats / columns);
    var matrix = [];
    var chars = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J'];
    for (var r = 1; r <= rows; r++) {
        var rowSeats = [];
        for (var c = 0; c < columns; c++) {
            rowSeats.push("".concat(chars[c]).concat(r));
        }
        matrix.push(rowSeats);
    }
    return matrix;
}
export var seatMatrix = generateSeatMatrixForPlane(60);
/** Azul=libre por defecto; otros según diagrama (Venta=verde, Reserva=amarillo, Devolución=rojo). */
export var seatState = {
    A2: 'sold',
    B1: 'sold',
    C3: 'reserved',
    D5: 'sold',
